...
func TestCertificatesApiService_DownloadCertificates_WithoutValidator(t *testing.T) {
	...
	assert.Nil(t, result.Request.Body)

	require.Len(t, resp.Data, 1)

	assert.Equal(t, "D7CE59D1F522D701", *resp.Data[0].SerialNo)
	assert.Equal(t, "AEAD_AES_256_GCM", *resp.Data[0].EncryptCertificate.Algorithm)
	assert.Equal(t, "certificate", *resp.Data[0].EncryptCertificate.AssociatedData)
	assert.Equal(t, "3a584b49ed9b", *resp.Data[0].EncryptCertificate.Nonce)

	effectiveTime, err := time.Parse(time.RFC3339, "2021-04-27T16:55:23+08:00")
	require.NoError(t, err)
	assert.Equal(t, effectiveTime, *resp.Data[0].EffectiveTime)

	expireTime, err := time.Parse(time.RFC3339, "2031-04-25T16:55:23+08:00")
	require.NoError(t, err)
	assert.Equal(t, expireTime, *resp.Data[0].ExpireTime)
}
